...
- Confirm every macOS DMG has been mounted and verified **locally/on a capable macOS host** before normalization. CI checks only that exactly one `.dmg` artifact was created and explicitly reports its `--skip-mount` boundary; this is never mounted-layout proof. Use `python desktop/scripts/verify_dmg_layout.py --dmg <path-to.dmg> --require-hdiutil` for real layout acceptance. The visible root must contain `足球赛事 AI 推演引擎.app` and may contain `Applications`; it must not contain bare `Contents/`. Never accept `hdiutil convert` success alone.
